...
def _saveFig(zfile, iname, fig):
    ...
    if fig.canvas is None:
        raise Exception("Cannot find canvas. Try resurecting the figure first.")
    for ax in fig.axes:
        disable(ax)
...
def _createFigZip(filename, force, callback):
    ...
    with zipfile_factory(filename, mode="w", compression=zipfile.ZIP_DEFLATED) as zfile:
        callback(zfile)
...
def engFormat(
    ax=None, axis="y", unit="", places=None, sep=" ", useOffset=False, *, norm_unit=True
):
    """
...
if using_pandas:
    ...
    def plotdf(d: pd.DataFrame, x="temp", ax=None, xd=None, groupby=[], **kwargs):
        """
        ...
        unstack = [name for name in d.index.names if name != x and name not in groupby]
        if ax is None:
            ax = plt.gca()
        d = d.unstack(unstack)
    ...
    def plotdf2(x, y, data, ax=None, hue=None, shape=None, fit_order=False):
        ...
        for key in itertools.product(*keys):
            ...
            filt = functools.reduce(
                operator.and_, itertools.starmap(operator.eq, (zip(key, keys))), x == x
            )
            if not any(filt):
                continue
            fx = x[filt]
            ...
            ikey = iter(key)
            color = None if hue is None else colors[next(ikey)]
            marker = "." if shape is None else shapes[next(ikey)]

            label = key if len(key) > 1 else key[0]
            (sc,) = ax.plot(
                fx, fy, linestyle="", marker=marker, color=color, label=label
            )
            color = sc.get_color()
            ...
            if fit_order:
                ...
                fitlabel = " + ".join(
                    f"{a:.4g}{power(i)}" for i, a in enumerate(fit.convert().coef)
                )
                (line,) = ax.plot(fitx, fit(fitx), color=color, label=fitlabel)
        legtitle = tuple(legtitle)
        legtitle = legtitle if len(legtitle) > 1 else legtitle[0]
...
class EngFormatter(ticker.EngFormatter):
    ...
    def __call__(self, x, pos=None):
        if np.nextafter(x, self.offset) == self.offset:
            xp = 0
        else:
            xp = x - self.offset
        return super().__call__(xp, pos)
    ...
    def get_offset(self):
        ...
        offsetStr = ""
        if self.offset:
            offsetStr = super().__call__(self.offset)
            if self.offset > 0:
                offsetStr = "+" + offsetStr
...
class TimeFormatter(EngFormatter):
    ...
    def format_time(secs):
        ...
        if hours:
            return "{:s}{:d}:{:0>2d}:{:0>2d} h".format(
                s, int(hours), int(mins), int(secs + 0.5)
            )
        return "{:s}{:d}:{:0>2d}.{:1d} m".format(
            s, int(mins), int(secs), int(10 * subsecs + 0.5)
        )
...
class ZoomControl:
    ...
    def scroll_fun(self, event):
        ...
        (cur_xlim, cur_ylim) = self.to_log(self.ax.get_xlim(), self.ax.get_ylim())
    ...
    def press_fun(self, event):
        ...
        if event.button == MouseButton.LEFT and self.movable and not self.drawing:
    ...
    def stop_draw(self, event):
        ...
        self.ax.set_xlim([min(self.startx, self.stopx), max(self.startx, self.stopx)])
        self.ax.set_ylim([min(self.starty, self.stopy), max(self.starty, self.stopy)])
...
class ToggleControl:
    ...
    def __init__(self, ax):
        ...
        self.ax.legend = wrap(self.ax.legend, before=saveHandles, after=doUpdate)
    ...
    def updateDict(self):
        ...
        for legline, origline, mappings in zip(leg.get_lines(), lines, mapping):
...
class MarkerControl:
    ...
    def __init__(self, ax):
        ...
        self.markers = defaultDictKey(lambda name: Marker(ax, self.formatter, name))
    ...
    @staticmethod
    def formatter(x, y, ax=None, delta=False, verbose=False):
        if ax is not None:
                ...
                if verbose:
                    ...
                    if xunit == 1 and yunit == 1:
                        ...
                        dxyformat = EngFormatter(
                            unit=UNITS_DIV.get((xunit, yunit), f"[{xunit}/{yunit}]")
                        )
                        dyxformat = EngFormatter(
                            unit=UNITS_DIV.get((yunit, xunit), f"[{yunit}/{xunit}]")
                        )
    ...
    def place_marker(self, event, marker, snap=True):
        ...
        marker.xy = x, y
        marker.set_text(self.formatter(x, y, self.ax))
    ...
    def snap(self, x, y):
        try:
            data = np.hstack([x.get_data() for x in self.ax.lines if x.get_visible()]).T
